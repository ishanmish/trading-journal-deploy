...
const TradingHeatmap = ({ entries }) => {
    ...
    return (
        <div className="bg-gray-900 p-6 rounded-xl border border-gray-700 w-full mb-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <h3 className="text-gray-400 font-semibold uppercase tracking-wider text-sm">Performance View</h3>

                <div className="flex bg-gray-800 p-1 rounded-lg border border-gray-700 gap-2">
                    {/* Period Toggle */}
                    <div className="flex items-center gap-1 border-r border-gray-700 pr-2 mr-1">
                        <button
                            onClick={() => setPeriod('weekly')}
                            className={`px-3 py-1 rounded text-xs font-bold transition-all ${period === 'weekly' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
                        >
                            Weekly
                        </button>
                        <button
                            onClick={() => setPeriod('monthly')}
                            className={`px-3 py-1 rounded text-xs font-bold transition-all ${period === 'monthly' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}
                        >
                            Monthly
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className={`px-3 py-1 rounded text-xs font-bold transition-all ${viewMode === 'heatmap' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}
                        >
                            Heatmap
                        </button>
                        <button
                            onClick={() => setViewMode('bar')}
                            className={`px-3 py-1 rounded text-xs font-bold transition-all ${viewMode === 'bar' ? 'bg-purple-600 text-white' : 'text-gray-400 hover:text-white'}`}
                        >
                            Bar Chart
                        </button>
                    </div>
            ...
            <div className={`grid gap-2 pb-4 ${viewMode === 'heatmap' ? 'grid-cols-[repeat(auto-fill,125px)] content-start' : 'grid-cols-1 overflow-x-auto min-w-full'}`}>
                {viewMode === 'heatmap' ? (
                    items.map((itemDate, index) => {
                        ...
                        return (
                            <div key={index} className="flex flex-col w-max bg-gray-900/40 p-3 rounded-lg border border-gray-800/50 hover:border-gray-600 transition-colors">
                                <div>
