...
const JournalEntryForm = ({ isOpen, onClose, onSuccess, editingDate, prefillData }) => {
    ...
    useEffect(() => {
        if (editingDate && isOpen) {
            setIsLoading(true);
            client.get(`/journal/daily_log/${editingDate}`)
                .then(res => {
                    const data = res.data;
                    setDate(data.date);
                    setNotes(data.notes || '');
            ...
            // Pre-fill from broker API fetch
            ...
            const updatedAccounts = DEFAULT_ACCOUNTS.map(name => {
                const fetched = prefillData.accounts?.find(a => a.account_name === name);
                return {
                    account_name: name,
                    pnl: fetched ? fetched.pnl : '',
                    brokerage: fetched ? fetched.brokerage : '',
                    taxes: fetched ? fetched.taxes : ''
                };
            });
            setAccounts(updatedAccounts);
    ...
    }, [editingDate, isOpen, prefillData]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedImages.length > 0) {
            ...
            try {
                const uploadRes = await client.post('/journal/upload_images', formData, {
        ...
        try {
            await client.post('/journal/daily_log', payload);
                                    ...
                                    <div className="flex flex-wrap gap-4">
                                        ...
                                        {existingImages.map((src, i) => (
                                            <div key={`existing-${i}`} className="relative group w-32 h-32 rounded-lg overflow-hidden border border-gray-600">
                                                <img src={`${API_URL}/${src}`} alt="Evidence" className="w-full h-full object-cover" />
                                                <button
                                                    type="button"
