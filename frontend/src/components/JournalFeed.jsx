        ...
        <div className="space-y-4">
            {groupedByDate.map((dayData, index) => (
                <motion.div
                    ...
                    className="bg-gradient-to-b from-[#1e1e1e] to-[#0a0a0a] p-6 rounded-3xl border border-[#262626] hover:border-[#404040] transition-all mb-4"
                >
                    ...
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex gap-1">
                                <button
                                    ...
                                    className="p-1.5 text-[#52525b] hover:text-blue-400 hover:bg-[#262626] rounded transition-colors"
                                    title="Edit"
                                >
                                    ✏️
                                </button>
                                <button
                                    ...
                                    className="p-1.5 text-[#52525b] hover:text-red-400 hover:bg-[#262626] rounded transition-colors"
                                    title="Delete"
                                >
                                    🗑️
                                </button>
                        ...
                        <div className={`text-2xl font-bold ${dayData.totalPnl >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                        {dayData.accounts.map((acc, i) => (
                            <div key={i} className="bg-black px-3 py-2 rounded-lg text-sm flex items-center gap-2 border border-[#262626]">
                                <span className="text-[#a1a1aa] font-medium">{acc.account_name}</span>
                                <span className={acc.pnl >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}>
                    ...
                    {/* Notes */}
                    {dayData.notes && (
                        <div className="text-[#d4d4d8] text-sm mb-4 whitespace-pre-wrap bg-[#18181b] p-3 rounded-lg border border-[#262626]">
                            {dayData.notes}
                        </div>
                    )}
                        ...
                        <div className="mb-4">
                            <div className="flex flex-wrap gap-2">
                                {dayData.image_paths.map((path, i) => (
                                    <div
                                        key={i}
                                        className="relative group w-32 h-32 rounded-lg overflow-hidden border border-[#262626] cursor-pointer"
                    ...
                    {dayData.twitter_logs && dayData.twitter_logs.length > 0 && (
                        <div className="mt-4 pt-4 border-t border-[#262626]">
                            <h4 className="text-xs text-[#52525b] uppercase tracking-wider mb-2">Followed Accounts</h4>
                            <div className="flex flex-wrap gap-2">
                                {dayData.twitter_logs.map((log, i) => (
                                    <div key={i} className="bg-black px-3 py-1 rounded-lg text-xs flex items-center gap-2 border border-[#262626]">
                                        <span className="text-blue-400">@{log.twitter_handle}</span>
                                        <span className={log.pnl >= 0 ? 'text-[#4ade80]' : 'text-[#f87171]'}>
                                            {log.pnl >= 0 ? '+' : ''}{log.pnl}%
                                        </span>
                                    </div>
                                ))}
