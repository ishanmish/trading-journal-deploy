...
const MarketContextCard = ({ date }) => {
    ...
    const renderIndex = (name, indexData) => {
        if (!indexData || indexData.status === 'no_data') {
            return (
                <div className="min-w-[180px] bg-gradient-to-b from-[#1e1e1e] to-[#0a0a0a] rounded border border-[#262626] p-3 flex flex-col">
    ...
    if (loading) {
        return (
            <div className="flex gap-4">
                <div className="min-w-[180px] h-20 bg-gradient-to-b from-[#1e1e1e] to-[#0a0a0a] rounded border border-[#262626] animate-pulse" />
                <div className="min-w-[180px] h-20 bg-gradient-to-b from-[#1e1e1e] to-[#0a0a0a] rounded border border-[#262626] animate-pulse" />
            </div>
        );
    }
